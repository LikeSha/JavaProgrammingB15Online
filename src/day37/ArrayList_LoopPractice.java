...
public class ArrayList_LoopPractice {
    ...
    public static void main(String[] args) {
        ...
        String longestNmae = "";// String longestName = nameLst.get(0);
        ...
        for (int x = 0; x < nameLst.size() ; x++) {
            ...
            if( currentName.length() > longestNmae.length()){
                longestNmae = currentName ;
            }

        }
        System.out.println("longestNmae = " + longestNmae);
        ...
        // then loop through each names and check whether character count
        // equal to longest word character count
        for (int i = 0; i <nameLst.size() ; i++) {

            if(nameLst.get(i).length() == longestNmae.length()){
