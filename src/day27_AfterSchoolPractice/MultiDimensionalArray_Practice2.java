...
public class MultiDimensionalArray_Practice2 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("shortestString = " + shortestString);
         ...
         // practice myself : find the second shortest string
        int secondLength = scrumTeam[0][0].length() ;
        String secondShort = scrumTeam[0][0] ;
        for (int i = 0; i <scrumTeam.length ; i++) {
            for (int j = 0; j <scrumTeam[i].length ; j++) {
                if(secondLength > scrumTeam[i][j].length() && secondShort != shortestString);
                secondLength = scrumTeam[i][j].length();
                secondShort = scrumTeam[i][j] ;
            }
        }
